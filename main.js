...
8
 ...
 const productsWithPrice = products.map((product, i) => {
    return {
      product: product,
      precio: precios[i]
    };
...
console.log(sixCharCountriesOrMore);
 ...
 const countriesFilteredByE = countriesData.filter((country) => {
    return countriesData.toLowerCase().startsWith('e')
 })
 console.log(countriesFilteredByE);

 const paisesFiltrados = paises.filter(pais => pais.startsWith('E'));

const pricesWithValue = products.filter((product) => product.price ===undefined && product.price == isNaN && product.price === null);
console.log(pricesWithValue)

const showArrayElements = (array = "Angel") => {
...
const showMapElements = (array) => {
    array.map((element) => console.log(element))
}

const namesWithI = names.filter((name)=> name.toLowerCase().includes("i"))
